...
**Status: enforced.** It was reported-only while the apps migrated — findings printed as
`pending R16 …` and failed nothing, behind an `R16_ENFORCED` constant — because turning it on
before the layout moved would have failed every open PR in four repositories over a shape none of
them had. All five apps conformed on their default branches on 2026-08-17, so the gate came out
rather than being set to `True`: a switch nobody will flip again is dead configuration, and this
rule now runs like every other one. [Incidents §R16](docs/CONFORMANCE-INCIDENTS.md#r16--the-app-bundles-inputs-live-in-app)
keeps the record of what the gated period was for and what it caught. It is not a skip: the findings are computed and printed on every run, and
