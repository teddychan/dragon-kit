...
def make_app(tmp: str, *, menu: str = COMPLIANT_MENU, panes: str = COMPLIANT_PANES,
    ...
    # Every fixture is R16-conforming by default. R16 is an ordinary failing rule, so without this
    # every unrelated fixture in the suite would fail on a layout none of them was written to test.
...
def load_checker():
    ...
    observable from outside it: `mask_noncode`'s length and line-count invariants.
    """
...
"""},
        ...
        # ice-2's route: an empty placeholder the release workflow fills. The plist goes in `App/`,
        # not at the root — this fixture is about §R12's stamping route, and a root plist now trips
        # §R16 as collateral. Which is the point of `expect_pass` being strict about the whole run:
        # a fixture that passes its own rule while failing another proves nothing about either.
        expect_pass("stamped via a placeholder in Info.plist", make_app(
            tmp, build="# stamped by the release workflow\n",
            extra={"App/Info.plist": "<key>DragonCommitDate</key><string></string>\n"}))
...
"""}), "R15",
        ...
        print("R16 — bundle inputs live in App/")
        ...
        expect_violation("plist at the repo root", make_app(tmp, plist_path="Info.plist"),
            "R16", because="no 'App/' directory at the repo root")
        ...
        expect_violation("lowercase app/ is not App/", make_app(tmp, plist_path="app/Info.plist"),
            "R16", because="found 'app/'")
        expect_violation("a bundle input outside App/", make_app(
            tmp, extra={"Ice/Resources/Info.plist": COMPLIANT_PLIST}),
            "R16", because="bundle input outside 'App/'")
        expect_violation("an icon outside App/", make_app(tmp, extra={"AppIcon.icns": "icns"}),
            "R16", because="bundle input outside 'App/'")
        ...
        expect_violation("entitlements outside App/", make_app(
        ...
        expect_violation("a helper plist nested too deep", make_app(
            tmp, extra={"App/Helpers/MenuBarItemService/Info.plist": COMPLIANT_PLIST}),
            "R16", because="bundle input outside 'App/'")
        expect_violation("no main-bundle plist at all", make_app(
            tmp, plist_path=None, extra={"App/AppIcon.icns": "icns"}),
            "R16", because="no 'App/Info.plist'")
...
"""))
        ...
        # R16 is a whole-app check, so it accepts an app-wide exception and rejects a path-scoped
        # one — the same shape as §R5, §R8, §R9 and §R12. Both directions are asserted: without the
        # second, dropping R16 from APP_WIDE_ONLY_RULES would silently start accepting a path that
        # suppresses nothing while printing as a live sanction, which is §R11's own incident.
        expect_pass("an app-wide R16 exception suppresses it", make_app(
            tmp, plist_path="Info.plist", config_over={"exceptions": [{
                "rule": "R16",
                "reason": "vendored upstream layout this app does not control",
                "sanctionedBy": "CONFORMANCE.md §R11"}]}))
        ...
        expect_violation("a lowercase filename is not Info.plist", make_app(tmp, plist_path=None,
